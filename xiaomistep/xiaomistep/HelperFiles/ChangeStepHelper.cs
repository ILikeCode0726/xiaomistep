...
namespace xiaomistep.HelperFiles
{
    ...
    public class ChangeStepHelper
    {
        ...
        public async Task<bool> Start(string account, string pwd, int step)
        {
            ...
            string code = await LoginAndGetCode(account, pwd, isEmail);
            if (string.IsNullOrEmpty(code))
            {
                LogsHelper.Error("账号:" + account + "执行失败1，账号或者密码不对");
                return false;
            }
            (string, string) temp = await GetToken(code, isEmail);
        ...
        public static HttpClient CreatHttpClient(List<(string, string)> headers)
        {
        ...
        public static async Task<string> LoginAndGetCode(string account, string pwd, bool isEmail)
        {
            string urlPar = "+86";
            if (isEmail)
            {
            ...
            HttpContent postContent = new FormUrlEncodedContent(new Dictionary<string, string>()
           {
            ...
            var temp = await httpClient.PostAsync($"https://api-user.huami.com/registrations/{urlPar}{account}/tokens", postContent);
            //登录成功
        ...
        async Task<(string, string)> GetToken(string code, bool isEmail)
        {
            HttpClient httpClient = CreatHttpClient(new List<(string, string)>() { ("UserAgent", "MiFit/4.6.0 (iPhone; iOS 14.0.1; Scale/2.00)") });
            HttpContent postContent = new FormUrlEncodedContent(new Dictionary<string, string>()
            {
        ...
        {
            long last_sync_data_time = await TimeHelper.Get1970ToNowSecondsAsync() - 13081;//13081=3小时38分钟1秒，now-1970.1.1在往前推几小时
